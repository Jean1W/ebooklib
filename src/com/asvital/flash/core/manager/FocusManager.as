...
package com.asvital.flash.core.manager {
	import com.asvital.flash.core.display.BaseDispaly;
	import com.asvital.flash.core.display.Button;
	...
	public class FocusManager implements IFocusManager {
		...
		private var _focusBorder:Boolean = true;
		public function FocusManager(container:DisplayObjectContainer) {
			...
			if(container != null) {
				...
				addFocusTarget(_form);
				
				form.addEventListener(Event.ADDED,onTargetAddHandler,false,0,true);
				form.addEventListener(Event.REMOVED,onTargetRemovedHandler,false,0,true);
				form.stage.addEventListener(KeyboardEvent.KEY_DOWN, keyDownHandler, true, 0, true);
				form.stage.addEventListener(KeyboardEvent.KEY_UP, keyUpHandler, true, 0, true);
				//form.stage.addEventListener(
				//InteractiveObject(form).addEventListener(
			}
		}
		...
		protected function keyUpHandler(event:KeyboardEvent):void
		{
			...
			if(lastFocus){
				this.lastFocus.dispatchEvent(event.clone());
			}
		}
		
		protected function onTargetRemovedHandler(event:Event):void
		{
			
			//trace(event.target,event.currentTarget);
			//trace(focusableCandidates.indexOf(event.target));
			removedFocusTarget(event.target as DisplayObject);
		}
		
		private function keyDownHandler(event:KeyboardEvent):void
		{
			//trace(lastFocus.name,event.currentTarget.name,event.target.name);			
			//过滤 来自 舞台的重复调用。
			if(lastFocus==event.target){
				return;
			}			
			if(lastFocus){
				var minLen:Number =Number.MAX_VALUE;
				var selectIndex:int = -1;
				var bRect:Rectangle = lastFocus.getBounds(lastFocus.parent);
				
				//var b:Point = lastFocus.localToGlobal(new Point(bRect.x+bRect.width/2,bRect.y+bRect.height/2));
				var b:Point = lastFocus.localToGlobal(new Point(bRect.x,bRect.y));
				var i:int = 0;
				...
				if(event.keyCode == Keyboard.ENTER){
					lastFocus.dispatchEvent(new Event(Event.SELECT));
				}else if(event.keyCode == Keyboard.TAB){
					//trace(event.target);
					//trace(form.stage.focus==event.target);
					event.preventDefault();
					return;
					io = event.target as InteractiveObject;
					
					if(io && (io is Stage)==false && io["buttonMode"]){
						this.setFocus(io);
						//io.addEventListener(FocusEvent.FOCUS_OUT,onTabFocusOutHandler);
					}
					//lastFocus.dispatchEvent(new Event(Event.CLOSE));
					...
					for(i=0;i<focusableCandidates.length;i++){	
						...
						io = focusableCandidates[i];
						a = io.localToGlobal(new Point(io.x,io.y));
						
						
						if(io && a.x>=b.x && lastFocus!=io && io["buttonMode"] && isEnabledAndVisible(io)){
							
							len = Point.distance(a,b);
							if(minLen>len){
								minLen = len;
								selectIndex = i;
							}
					...
					//var index:int = focusableCandidates.indexOf(lastFocus)+1;
					if(selectIndex>=0){
						this.setFocus(focusableCandidates[selectIndex]);
					}
		...
		protected function onTabFocusOutHandler(event:FocusEvent):void
		{
			//io.addEventListener(FocusEvent.FOCUS_OUT,onTabFocusOutHandler);
			// TODO Auto-generated method stub
			trace(event.relatedObject);
			this.setFocus(event.relatedObject);
		}
		
		protected function onTargetAddHandler(event:Event):void
		{
			
			//trace(event.target is InteractiveObject,event.currentTarget);
			addFocusTarget(event.target as DisplayObject);			
			
			
		}
		private function removedFocusTarget(con:DisplayObject):void{
			var o:DisplayObjectContainer = (con as DisplayObjectContainer);
			
			if(o){
				o.focusRect = false;
				//trace(o.hasOwnProperty("buttonMode"));
				trace(focusableCandidates.indexOf(o));
				
				
				if(o["buttonMode"]){
					trace((o).name,"---");
					focusableCandidates.splice(focusableCandidates.indexOf(o),1);
				}
				for(var i:int=0;i<o.numChildren;i++){					
					//trace(con.getChildAt(i));					
					this.removedFocusTarget(o.getChildAt(i));
					
				}
				
			}else{
				if(con is InteractiveObject){
					//InteractiveObject(con).focusRect = false;
					//trace(con["buttonMode"]);
					trace(focusableCandidates.indexOf(o));
					
					if(con["buttonMode"]){
						//focusableCandidates.push(o);
						trace((con).name,"---");
						focusableCandidates.splice(focusableCandidates.indexOf(o),1);
					}
				}
		...
		private function addFocusTarget(con:DisplayObject):void{
			...
			if(o){
				...
				//trace(o.name);
				if(o["buttonMode"]){
					if(focusableCandidates.indexOf(o)==-1){
						focusableCandidates.push(o);
					}
				...
				for(var i:int=0;i<o.numChildren;i++){					
					//trace(con.getChildAt(i));					
					this.addFocusTarget(o.getChildAt(i));
		 ...
		 *  @productversion Flash CS3
		 */
		private var kk:Shape = new Shape();
		public function setFocus(component:InteractiveObject):void {
			...
			if(component is InteractiveObject && component["buttonMode"]==true  &&  component.visible && isEnabledAndVisible(component)) {
				kk.graphics.clear();
				kk.graphics.lineStyle(2,0xff00ff);
				
				if(lastFocus){
					lastFocus.dispatchEvent(new FocusEvent(FocusEvent.FOCUS_OUT));
				}
				
				this.lastFocus = component;
				
				component.dispatchEvent(new FocusEvent(FocusEvent.FOCUS_IN));
				
				var rect:Rectangle = component.getBounds(component.parent);
				var op:Point = component.parent.localToGlobal(new Point(component.x,component.y));
				//trace(rect);
				op = form.globalToLocal(op);
				//IFocusManagerComponent(component).setFocus();
				kk.graphics.drawRect(op.x,op.y,rect.width,rect.height);
				kk.graphics.endFill();
				//kk.x = op.x;
				//kk.y = op.y;
